...
void DrawObj(ObjModel model, Vector3<double> ShiftOrigin = Vector3<double>(0.0, 0.0, 0.0), bool filled_polygons=false);
...
void DrawLine(Vector2<int> a, Vector2<int> b, Vector3<unsigned char> color = Vector3<unsigned char>(0xff, 0xff, 0xff), bool markToFill = false); // Draws line, Bresenham's line algorithm
...
void TestDrawCircle(Vector2<int> a, int radius, double step, Vector3<unsigned char> color = Vector3<unsigned char>(0xff, 0xff, 0xff));
void TestDrawFan(Vector2<int> a, int radius, double step, Vector3<unsigned char> color = Vector3<unsigned char>(0xff, 0xff, 0xff));
